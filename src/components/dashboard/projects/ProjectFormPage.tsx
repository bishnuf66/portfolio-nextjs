...
const ProjectFormPage: React.FC<ProjectFormPageProps> = ({
  ...
  const galleryUploadControllers = useRef<Map<string, AbortController>>(
    new Map()
  );
  ...
  const handleCoverImageChange = async (file: File) => {
    ...
    try {
      const uploadedUrl = await handleImageUpload(
        file,
        coverUploadController.current.signal
      );
  ...
  const removeCoverImage = async () => {
    ...
    if (uploadStates.coverUploading) {
      ...
      if (
        currentCoverUrl &&
        initialData?.slug &&
        !currentCoverUrl.startsWith("blob:")
      ) {
        try {
          const token = localStorage.getItem("supabase_token");
          const response = await fetch(
            `/api/projects/${initialData.slug}/images?url=${encodeURIComponent(
              currentCoverUrl
            )}`,
            {
              method: "DELETE",
  ...
  const removeGalleryImageWithTitle = async (index: number) => {
    ...
    setFormData((prev) => ({
      ...
      gallery_images_with_titles:
        prev.gallery_images_with_titles?.filter((_, i) => i !== index) || [],
    }));
  };
  ...
  const updateGalleryImageTitle = (index: number, title: string) => {
    setFormData((prev) => ({
      ...prev,
      gallery_images_with_titles:
        prev.gallery_images_with_titles?.map((img, i) =>
          i === index ? { ...img, title } : img
        ) || [],
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const finalGalleryUrls = finalGalleryImagesWithTitles.map(
        (img) => img.url
      );
  ...
  const isAnyUploadInProgress =
    uploadStates.coverUploading || uploadStates.galleryUploading;
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                <input
                  ...
                  className={`w-full px-3 py-2 border rounded-lg file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold ${
                    isDarkMode
                  ...
                  <div className="relative inline-block">
                    <Image
                      ...
                      className={`w-32 h-32 object-cover rounded border ${
                        uploadStates.coverUploading ? "opacity-75" : ""
                      }`}
                    />
                    <button
                      type="button"
            ...
            <div>
              ...
              <div className="space-y-3">
                        ...
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updatePendingGalleryTitle(index, e.target.value)
                              }
                ...
                {formData.gallery_images_with_titles &&
                  formData.gallery_images_with_titles.length > 0 && (
                    <div>
                      <p className="text-sm font-medium mb-2">
                        Current Gallery Images:
                      </p>
                      <div className="space-y-3">
                        {formData.gallery_images_with_titles.map(
                          (imageWithTitle, index) => (
                            <div
                              key={index}
                            ...
                            >
                              <div className="relative">
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    removeGalleryImageWithTitle(index)
                                  }
                                  className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 hover:bg-red-600 transition-colors"
